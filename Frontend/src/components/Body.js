...
const Body = () => {
  ...
  const fetchData = async () => {
    const data = await fetch(
      "https://www.swiggy.com/dapi/restaurants/list/v5?lat=12.9310903&lng=80.15262709999999&is-seo-homepage-enabled=true&page_type=DESKTOP_WEB_LISTING"
    );
  ...
  const filterSearch = () => {
    const filter = listofRes.filter((item) =>
  ...
  return (
    <div className="p-4">
      <div className="flex">
        ...
        <div className="pl-16">
          <button
            className="px-3 py-1 text-black bg-gray-200 hover:bg-gray-300 rounded-md"
      ...
      <div className="flex flex-wrap">
        {filteredRestro.map((item) => (
          <Link key={item.info.id} to={"/restromenu/" + item.info.id}>
